package CoreParts;//TIP To <b>Run</b> code, press <shortcut actionId="Run"/> or
// click the <icon src="AllIcons.Actions.Execute"/> icon in the gutter.

import CoreParts.impl.EngineImpl;
import CoreParts.smallParts.CellLocation;
import jakarta.xml.bind.JAXBException;

import java.io.FileNotFoundException;

public class Main {
    public static void main(String[] args) {
        EngineImpl engine = new EngineImpl();

        engine.updateCell("10", 'A', '1');
        engine.updateCell("8", 'A', '3');
        engine.updateCell("{PLUS, {REF, A1}, {REF, A3}}", 'A', '2');
        engine.updateCell("{SUB, Goodbye And Hello, {REF, A1}, {REF, A3}}", 'B', '1');
        System.out.println("check!!");

        engine.updateCell("10", 'A', '4');
        //engine.updateCell("{PLUS, {REF, A1}, {REF, A2}}", 'A', '3');
        System.out.print(engine.getCell(CellLocation.fromCellId('A', '1')).getEffectiveValue().evaluate().getValue() + "\n");



    }
}